...
tmp_home="$(mktemp -d "${TMPDIR:-/tmp}/sal-cdr-smoke.XXXXXX")"
cleanup() {
  rm -rf "$tmp_home"
}
trap cleanup EXIT
...
run_case() {
  ...
  echo "[INFO] $label"
  output=$(HOME="$tmp_home" "$SAL_CDR" "$@" 2>&1)
  printf '%s\n' "$output"
...
run_case "sal-cdr nonlinear_square nonnegative" --solver=yices2 --to=2 nonlinear_square nonnegative
run_case "sal-cdr nonlinear_square bad_bound" --solver=yices2 --to=2 nonlinear_square bad_bound
run_case "sal-cdr nonlinear_product positive" --solver=yices2 --to=2 nonlinear_product positive
run_case "sal-cdr nonlinear_product bad_bound" --solver=yices2 --to=2 nonlinear_product bad_bound
